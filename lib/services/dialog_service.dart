...
class DialogService {
  ...
  Future<List<DialogModel>> getDialogs(String currentUserId) async {
    final records = await _pb
        .collection('dialogs')
        .getFullList(expand: 'users', sort: '-updated');
  ...
  Future<void> createDialog({
    ...
    final existing = await _pb
        .collection('dialogs')
        .getFullList(
          filter: "users ~ '$currentUserId' && users ~ '$otherUserId'",
        );
  ...
  void subscribe({
    required String currentUserId,
    required Function(DialogModel dialog) onCreate,
  }) {
    _pb.collection('dialogs').subscribe('*', (event) {
      ...
      onCreate(DialogModel.fromRecord(record, currentUserId));
    });
  }
